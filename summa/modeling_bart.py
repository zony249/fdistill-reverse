...
class BartEncoder(BartPretrainedModel):
    ...
    def forward_with_forced_inputs(
        ...
        r"""
        input_embeds: torch.Tensor[batch, layers, seq_len, hidden]
        """
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        ...
        if inputs_embeds is None:
            # inputs_embeds = self.embed_tokens(input_ids) * self.embed_scale
            raise ValueError("input_embeds must be supplied")

        embed_pos = self.embed_positions(input_shape)

        hidden_states = inputs_embeds[:, 0] + embed_pos
        hidden_states = self.layernorm_embedding(hidden_states)
        hidden_states = F.dropout(hidden_states, p=self.dropout, training=self.training)
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                if getattr(self.config, "gradient_checkpointing", False) and self.training and False:
...
class BartDecoder(BartPretrainedModel):
    ...
    def forward_with_forced_inputs(
        ...
        if inputs_embeds is None:
            # inputs_embeds = self.embed_tokens(input_ids) * self.embed_scale
            raise ValueError("inputs_embeds must be defined")
        ...
        hidden_states = inputs_embeds[:, 0] + positions
        hidden_states = self.layernorm_embedding(hidden_states)
