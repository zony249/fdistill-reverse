...
class MetricsSuite: 
    ...
    def compute_accum(self, enc_hidden_1, dec_hidden_1, enc_hidden_2=None, dec_hidden_2=None): 
        ...
        """
        dec_indices = [0] + [i+1 for i in LAYER_MAP[self.config2.decoder_layers]] if self.config2 is not None else None
        enc_indices = [0] + [i+1 for i in LAYER_MAP[self.config2.encoder_layers]] if self.config2 is not None else None

        if self.reverse_enc: 
            enc_indices = enc_indices[::-1] if enc_indices is not None else None
        if self.reverse_dec: 
            dec_indices = dec_indices[::-1] if dec_indices is not None else None
    ...
    def save_figs(self, dirname): 
        ...
        fig, ax = plt.subplots(ncols=2, figsize=(10, 4), gridspec_kw={'wspace': 0.5})
        x_enc = torch.arange(self.config1.encoder_layers)
        x_dec = torch.arange(self.config1.decoder_layers)
        width = 0.35 
        ...
        if self.config2 is not None: 
            ...
            ax[0].bar(torch.tensor([i+1 for i in LAYER_MAP[self.config2.encoder_layers]]) + shift, student_between_enc_dist, width, label= "Student") 
            ax[1].bar(torch.tensor([i+1 for i in LAYER_MAP[self.config2.decoder_layers]]) + shift, student_between_dec_dist, width, label= "Student") 
...
def generate_summaries_or_translations(
    ...
    **generate_kwargs,
    ...
    compare_model = AutoModelForSeq2SeqLM.from_pretrained(compare_with_second_model).to(device) if compare_with_second_model is not None else None
    if fp16:
        model = model.half()
