...
class SummarizationDistiller(SummarizationModule):
    ...
    def __init__(self, hparams):
        ...
        self.do_calc_hidden_loss = False#(not self.different_base_models) and hparams.alpha_hid > 0
        ...
        self.max_gen_length = hparams.max_gen_length
    ...
    def calc_tvd_loss(self, mask, s_logits, t_logits):
        if 'log' in self.kd_method:
            s_logits = F.log_softmax(s_logits/self.temperature, dim=-1)    
            t_logits = F.log_softmax(t_logits/self.temperature, dim=-1)
        else:
            s_logits = F.softmax(s_logits/self.temperature, dim=-1)
            t_logits = F.softmax(t_logits/self.temperature, dim=-1)
        sel_mask = mask[:, :, None].expand_as(s_logits)
        vocab_size = s_logits.size(-1)
        s_logits_slct = torch.masked_select(s_logits, sel_mask)  # (bs * seq_length * voc_size) modulo the 1s in mask
        t_logits_slct = torch.masked_select(t_logits, sel_mask)  # (bs * seq_length * voc_size) modulo the 1s in mask
        ...
        t_logits_slct = t_logits_slct.view(-1, vocab_size)  # (bs * seq_length, voc_size) modulo the 1s in mask
        assert t_logits_slct.size() == s_logits_slct.size()
        loss_tvd = (0.5 * torch.abs(s_logits_slct-t_logits_slct)).sum(dim=-1).mean() * (self.temperature) ** 2
        return loss_tvd
    ...
    def _generate(self, batch: dict, gen_by_teacher=False):
        ...
        generated_ids = generated_ids[:, 1:] # remove the first eos token id
    ...
    def _js_teacher_step(self, batch):
        ...
        dec_mask = labels.ne(pad_token_id)
        loss_js = self.calc_js_loss(dec_mask, teacher_outputs["logits"], lm_logits, t_wght=self.beta, s_wght=(1.-self.beta))
