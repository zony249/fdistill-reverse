...
def bert_encode(model, x, attention_mask):
    model.eval()
    with torch.no_grad():
        # output, x_encoded_layers, _ = model(input_ids = x, attention_mask = attention_mask, output_hidden_states=True)
        out = model(input_ids = x, attention_mask = attention_mask, output_hidden_states=True)
...
def get_bert_embedding(all_sens, model, tokenizer, idf_dict,
    ...
    with torch.no_grad():
        for i in range(0, len(all_sens), batch_size):
            ...
            # batch_embedding = batch_embedding[0]
