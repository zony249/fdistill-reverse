...
#SBATCH --output=slurm-logs/slurm-%j-dart-768-mle-3l.out

nvidia-smi

export WIDTH=768
export TEACHER=models/dart-teacher-regularized/best_tfmr
export EXP_NAME=$(date +%m-%d-%y--%T)--dart-$WIDTH-mle-3l
export OUTPUT=$SCRATCH/fdistill-reverse/runs/dart/$EXP_NAME


python3 -m debugpy --listen 0.0.0.0:5678 distillation.py \
  --teacher $TEACHER \
  --data_dir dart \
  ...
  --alpha_hid=0. --alpha_mlm=1. --alpha_ce=1. \
