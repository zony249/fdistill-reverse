...
class DistillBertTrainer(Trainer): 
    ...
    def init_student_from_teacher(self, 
        ...
        if isinstance(student, BertForSequenceClassification): 
            ...
            student.bert.encoder.layer = deepcopy(nn.ModuleList([teacher.bert.encoder.layer[i] for i in layers_to_copy]))
            student.bert.pooler = deepcopy(teacher.bert.pooler)
        else:
            raise NotImplementedError()
    ...
    def _cosine_loss(self, 
        ...
        # basic loss
        cosines = [((t*s).sum(dim=-1) / (t.norm(dim=-1) * s.norm(dim=-1))) for t, s in zip(teacher_states, student_states)]
        losses = [1-c for c in cosines]
        # compute valid tokens (non-pad tokens)
    ...
    def _kl_loss(self, 
        ...
        """
        teacher_logits: Tensor[batch_size, class] 
        student_logits: Tensor[batch_size, class] 
        """

        p_x = teacher_logits 
        q_x = student_logits 

        loss = F.kl_div(F.log_softmax(q_x, dim=-1), F.softmax(p_x, dim=-1), reduction="batchmean") 
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        kl_loss = self._kl_loss(teacher_logits=teacher_outputs.logits, 
                          student_logits=student_outputs.logits) if self.compute_kl_loss else 0
...
if __name__ == "__main__": 
    ...
    class B(A):
        def __init__(self, a, d=4, e=5, *args, **kwargs): 
