    ...
    **generate_kwargs,
    ...
    for i, examples_chunk in tqdm(enumerate(list(chunks(examples, batch_size)))):
        ...
        with torch.no_grad():
            ...
            perturbed_outputs = model.encoder.forward_with_forced_inputs(inputs_embeds=perturbed, attention_mask=batch.attention_mask, output_hidden_states=True, return_dict=True).hidden_states
            ...
            perturbations = torch.normal(0, 1, size=(input_states.shape), device=model.device)
            perturbed = input_states + perturbations
            perturbed_outputs = model.decoder.forward_with_forced_inputs(
